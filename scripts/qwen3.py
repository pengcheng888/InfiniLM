...
from libinfinicore_infer.qwen3 import (
    MetaCStruct,
    WeightsCStruct,
    DataType,
    DeviceType,
...
class Qwen3Weights(WeightsCStruct):
    def __init__(self,
                 meta: Qwen3Meta,
                 state_dict: dict,
        ...
        super().__init__(nlayer, nh, nkvh, d, di, dh, ndev,
...
class Qwen3ForCauslLM:
    def __init__(
        ...
        if "qwen3" == self.config["model_type"] or "qwen2" == self.config["model_type"]:
            state_dict = load_all_safetensors_from_dir(model_dir_path)

            self.meta = Qwen3Meta(self.config, max_tokens=max_tokens)
