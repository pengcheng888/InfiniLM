...
from libinfinicore_infer_qwen2_moe import (
    MoEMetaCStruct,
    WeightsCStruct,
    DataType,
    DeviceType,
...
class Qwen2MoEMeta(MoEMetaCStruct):
...
class Qwen2MoEWeights(WeightsCStruct):
    def __init__(self,
                 meta: Qwen2MoEMeta,
                 state_dict: dict,
...
class Qwen2MoEForCauslLM:
    def __init__(
        ...
        if "qwen2_moe" == self.config["model_type"]:
            state_dict = load_all_safetensors_from_dir(model_dir_path)

            self.meta = Qwen2MoEMeta(self.config, max_tokens=max_tokens)
            self.weights = Qwen2MoEWeights(
                self.meta,
                state_dict,
                ndev=ndev,
        ...
        print(f"Qwen2MoEWeights, Time used: {load_end_time - load_start_time:.3f}s")
...
def test():
    ...
    model_path = r"/home/wangpengcheng/H100/Qwen1.5-MoE-A2.7B_small"
    model_path = r"/data-aisoft/wangpengcheng_data/Qwen1.5-MoE-A2.7B"
    
    ndev = int(sys.argv[3]) if len(sys.argv) > 3 else 1
    model = Qwen2MoEForCauslLM(model_path, device_type, ndev)
