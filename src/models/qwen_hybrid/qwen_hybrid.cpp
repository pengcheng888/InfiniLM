...
void inferDeviceBatch(const QwenHybridMeta *meta, DeviceResource &rsrc,
    ...
    for (uint32_t layer = 0; layer < nlayer; layer++) {
        ...
        linear(q_buf, logits_out,
               ...
               1.0, 0.0, nullptr, b_attn_q ? b_attn_q : nullptr);
        linear(k_buf, logits_out,
               weight->w_attn_k[layer],
               1.0, 0.0, nullptr, b_attn_k ? b_attn_k : nullptr);
        linear(v_buf, logits_out,
               weight->w_attn_v[layer],
               1.0, 0.0, nullptr, b_attn_v ? b_attn_v : nullptr);
        // rope
        ...
        // ------------------------------------------------------------------------ //
        {
            ...
            logits_in->debug("layer_ffn_out.bin");
        }
