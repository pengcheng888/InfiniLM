...
//                     auto k_req = qkv_rope->slice({{0, token_offset, seq_len}, {1, nh, nkvh}});
//                     auto v_req = qkv_rope->slice({{0, token_offset, seq_len}, {1, nh + nkvh, nkvh}});
...
//                     auto k_gemm = k_req->permute({1, 2, 0});
...
//                     auto v_gemm = v_req->permute({1, 0, 2});
...
//             }
//         } else {
//             // 非 paged attention：使用常规 KV cache
//             throw std::runtime_error("Non-paged attention not implemented");
//         }

//         // o_proj
//         linear(logits_in, o_buf, layer_tensor->self_attn->w_attn_out, 1.0, 0.0, idev == 0 ? logits_in : nullptr, nullptr);
...
//         // ------------------------------------------------------------------------
//         {
//             std::shared_ptr<Tensor> hidden_states = logits_out;
...
//             std::shared_ptr<Tensor> values_gpu = Tensor::buffer(infiniDtype_t::INFINI_DTYPE_F32, {ntok * topk}, rsrc.memory_pool);
//             std::shared_ptr<Tensor> indices_gpu = Tensor::buffer(infiniDtype_t::INFINI_DTYPE_I32, {ntok * topk}, rsrc.memory_pool);
