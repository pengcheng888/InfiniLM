...
void Qwen3MoEinferDeviceBatchPaged_old(const Qwen3MoE::Meta *meta,
                                       DeviceResource<Qwen3MoE::WeightsTensor> &rsrc,
                                       ...
                                       const float *topp,
                                       const uint32_t is_prefill,
                                       const bool enable_paged_attn,
                                       uint32_t *output,
                                       void *last_logits) {
    ...
    for (uint32_t ilayer = 0; ilayer < nlayer; ilayer++) {
        ...
        if (enable_paged_attn) {
            ...
            auto k_cache_pool = kv_caches[0]->k[idev][ilayer];
            auto v_cache_pool = kv_caches[0]->v[idev][ilayer];
            ...
            if (is_prefill) {
                ...
                for (uint32_t req = 0; req < nreq; req++) {
                    ...
                    // rearrange attn val
                    // printf("%s\n", o->info().c_str());
                    // printf("%s\n", attn_val_gemm->slice(2, 0, seq_len)->info().c_str());

                    auto temp = attn_val_gemm->slice(2, 0, seq_len);
                    rearrange(o, temp);

                    RUN_INFINI(infinirtDeviceSynchronize());
                    // o
                    // attn_val_gemm->slice(2, 0, seq_len)
                    o->debug(std::to_string(req) + "_o.bin");
                    temp->debug(std::to_string(req) + "_temp.bin");

                    token_offset += seq_len;
                    break;
                }

                throw std::runtime_error("Non-paged attention not implemented");
...
// (const Meta &, DeviceResource<WeightsTensor> &, uint32_t, uint32_t, const uint32_t *, uint32_t, const uint32_t *, uint32_t, const uint32_t *, struct KVCache **kv_caches, const int32_t *,
//  const int32_t *, const float *, const uint32_t *, const float *, uint32_t, bool, uint32_t *, void *))
void Qwen3MoEinferDeviceBatchPaged(const Qwen3MoE::Meta &meta,
                                   DeviceResource<Qwen3MoE::WeightsTensor> &rsrc,
                                   uint32_t idev, uint32_t ndev,
                                   const uint32_t *tokens, uint32_t ntok,
                                   const uint32_t *req_lens, uint32_t nreq,
    ...
    // 获取权重张量指针
    const Qwen3MoE::WeightsTensor *model_weights = rsrc.weights_tensor_ptr.get();
    if (!model_weights) {
        return; // 权重未加载，直接返回
    }
    ...
    bool has_qkv_bias = model_weights->layers[0]->self_attn->b_attn_qkv != nullptr;
    bool has_qk_norm = model_weights->layers[0]->self_attn->w_attn_qk_norm != nullptr;
    ...
    for (uint32_t i = 0; i < ntok; i++) {
        RUN_INFINI(infinirtMemcpyAsync(logits_in->data(i * d),
                                       model_weights->w_in_embd->data(tokens[i] * d),
                                       dsize(dt_logits) * d, INFINIRT_MEMCPY_D2D, stream));
    }
    ...
    for (uint32_t layer = 0; layer < nlayer; layer++) {
        ...
        auto layer_weights = model_weights->layers[layer];
        auto self_attn_weights = layer_weights->self_attn;
        // 1. Attention
        // rms norm
        rmsnorm(logits_out, logits_in, layer_weights->w_attn_norm, meta.epsilon);
        // qkv_proj
        linear(qkv_buf, logits_out, self_attn_weights->w_attn_qkv, 1.0, 0.0, nullptr, has_qkv_bias ? self_attn_weights->b_attn_qkv : nullptr);

        if (has_qk_norm) {
            rmsnorm(q_buf, q_buf, self_attn_weights->w_attn_qk_norm->slice(0, 0, dh), meta.epsilon);
            rmsnorm(k_buf, k_buf, self_attn_weights->w_attn_qk_norm->slice(0, dh, dh), meta.epsilon);
        }

        // rope
        rope(qkv_rope->slice(1, 0, nh), qkv_rope->slice(1, 0, nh), pos_ids_buf, model_weights->sin_table, model_weights->cos_table);
        rope(qkv_rope->slice(1, nh, nkvh), qkv_rope->slice(1, nh, nkvh), pos_ids_buf, model_weights->sin_table, model_weights->cos_table);
        ...
        // o_proj
        linear(logits_in, o_buf, self_attn_weights->w_attn_out, 1.0, 0.0, idev == 0 ? logits_in : nullptr, nullptr); // only rank 0 adds residual
    ...
    if (idev == 0) {
        if (last_logits != nullptr) {
            rmsnorm(logits_out, logits_in, model_weights->w_out_norm, meta.epsilon);
            auto last_logits_buf = Tensor::buffer(dt_logits, {ntok, dvoc}, rsrc.memory_pool);
            linear(last_logits_buf, logits_out, model_weights->w_out_embd, 1.0, 0.0, nullptr, nullptr);
        ...
        if (output != nullptr) {
            ...
            for (uint32_t req = 0; req < nreq; req++) {
                ...
                rmsnorm(logits_out->slice(0, req, 1),
                        logits_in->slice(0, token_offset - 1, 1),
                        model_weights->w_out_norm,
                        meta.epsilon);
            }
            linear(prob_buf, logits_out->slice(0, 0, nreq), model_weights->w_out_embd, 1.0, 0.0, nullptr, nullptr);
            std::random_device _rd;
            std::mt19937 gen(_rd());
            token_offset = 0;
            for (uint32_t req = 0; req < nreq; req++) {
...
void Qwen3MoEinferDeviceBatch(const Qwen3MoE::Meta *meta,
                              DeviceResource<Qwen3MoE::WeightsTensor> &rsrc,
                              uint32_t idev, uint32_t ndev,
                              const uint32_t *tokens, uint32_t ntok,
                              const uint32_t *req_lens, uint32_t nreq,
...
Model::Model(const Meta *_meta, const Weights *weights, infiniDevice_t device_, std::vector<int> device_ids) : meta(*_meta) {
    ...
    for (int i = 0; i < ndev; i++) {
        threads[i] = std::thread(launchDevice<WeightsTensor, Meta, Weights>,
                                 ...
                                 dev_ids[i], comms[i], Qwen3MoEinferDeviceBatch, Qwen3MoEinferDeviceBatchPaged);
    }
