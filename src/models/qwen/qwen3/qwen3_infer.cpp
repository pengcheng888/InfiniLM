...
void Qwen3inferDeviceBatch(const Qwen3::Meta *meta, DeviceResource &rsrc,
    ...
    const Qwen3::WeightsTensor *g_WeightsTensor{nullptr};
    if (rsrc.weights_tensor_ptr) {
        g_WeightsTensor = static_cast<const Qwen3::WeightsTensor *>(rsrc.weights_tensor_ptr);
    } else {
        return;
    }
