...
void Qwen2MoEinferDeviceBatch(const Qwen2MoE::Meta *meta, DeviceResource &rsrc,
    ...
    const Qwen2MoE::WeightsTensor *g_WeightsTensor{nullptr};
    if (rsrc.weights_tensor_ptr) {
        g_WeightsTensor = static_cast<const Qwen2MoE::WeightsTensor *>(rsrc.weights_tensor_ptr);
    } else {
        return;
    }
    ...
    for (uint32_t ilayer = 0; ilayer < nlayer; ilayer++) {
        ...
        // ------------------------------------------------------------------------ //
        {
            ...
            // 需要提前申请的缓存
            size_t moe_intermediate_size = meta->_moe_intermediate_size;
            ...
            size_t shared_expert_intermediate_size = meta->_shared_expert_intermediate_size;
            ...
            // 需要提前申请的缓存
            std::shared_ptr<Tensor> router_gate_output = Tensor::buffer(dt_logits, {ntok, meta->_num_experts}, rsrc.memory_pool); // 路由专家的 gate 的输出
            ...
            //      hidden_states 经过一个共享专家
            {
                // 输入: hidden_states
                // 输出: shared_states
                linear(shared_gate_up_buf, hidden_states, layer_tensor->ffn->_shared_expert->w_ffn_gate_up, 1.0, 0.0, nullptr, nullptr);
                swiglu(shared_gate_buf, shared_up_buf, shared_gate_buf);
                linear(shared_states, shared_gate_buf, layer_tensor->ffn->_shared_expert->w_ffn_down, 1.0, 0.0, nullptr, nullptr);

                //
                linear(shared_gate_output, hidden_states, layer_tensor->ffn->_shared_expert_gate_weight, 1.0, 0.0, nullptr, nullptr); // N,1
                // sigmoid(shared_gate_output, shared_gate_output); TODO!

                shared_gate_output = shared_gate_output->view_as(shared_states->shape(), {1, 0});
                // mul(shared_states, shared_gate_output, shared_states); TODO!
            }

            // (2) topk操作：
            //      hidden_states 先经过 gate_weight，得到 router_gate_output
            //      router_gate_output 进行 topk 操作
            linear(router_gate_output, hidden_states, ffn->_gate_weight, 1.0, 0.0, nullptr, nullptr); // 这一行的代码是正确的
            {
...
Model::Model(const Meta *_meta, const Weights *weights, infiniDevice_t device_, std::vector<int> device_ids) : meta(*_meta) {
    ...
    for (int i = 0; i < ndev; i++) {
        threads[i] = std::thread(launchDevice<WeightsTensor, Meta, Weights>, std::cref(meta), weights, &dev_resources[i], std::ref(states[i]), std::ref(req), device, i, ndev, dev_ids[i], comms[i], Qwen2MoEinferDeviceBatch);
    }
    for (int i = 0; i < ndev; i++) {
