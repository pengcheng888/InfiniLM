...
def validator(monkeypatch):
    ...
    lib_module._infinilm = types.SimpleNamespace(InferEngine=type("InferEngine", (), {}))
    ...
    spec = importlib.util.spec_from_file_location(
        module_name, package_root / "infer_engine.py"
    )
...
def test_valid_window(validator):
    validate, dtype = validator
    assert validate(make_tensor((1, 8), dtype), make_tensor((1, 8), dtype), 3) == (8, 3)


@pytest.mark.parametrize("name", ["input_ids", "labels"])
def test_requires_tensor_protocol(validator, name):
    validate, dtype = validator
    tensors = {"input_ids": make_tensor((1, 8), dtype), "labels": make_tensor((1, 8), dtype)}
    tensors[name] = object()
...
@pytest.mark.parametrize("name", ["input_ids", "labels"])
def test_requires_int64(validator, name):
    validate, dtype = validator
    tensors = {"input_ids": make_tensor((1, 8), dtype), "labels": make_tensor((1, 8), dtype)}
    tensors[name] = make_tensor((1, 8), dtype, dtype=object())
...
def test_rejects_invalid_shapes(validator, input_shape, label_shape, message):
    validate, dtype = validator
    with pytest.raises(ValueError, match=message):
        validate(make_tensor(input_shape, dtype), make_tensor(label_shape, dtype), 0)


@pytest.mark.parametrize("score_start", [-1, 8])
def test_rejects_empty_score_range(validator, score_start):
    validate, dtype = validator
    with pytest.raises(ValueError, match="select at least one token"):
        validate(make_tensor((1, 8), dtype), make_tensor((1, 8), dtype), score_start)


@pytest.mark.parametrize("score_start", [True, 1.5, "1"])
def test_requires_integer_score_start(validator, score_start):
    validate, dtype = validator
