...
Glm4MoeLiteMoE::Glm4MoeLiteMoE(std::shared_ptr<infinilm::config::ModelConfig> model_config,
    ...
    legacy_experts_ = this->register_module<infinilm::models::qwen3_moe::Qwen3MoeExperts>("experts", model_config, device);
    if (model_config->get_or<size_t>("n_shared_experts", 0) > 0) {
        shared_experts_ = this->register_module<Glm4MoeLiteMLP>(
            "shared_experts",
            make_shared_experts_config(model_config),
            device);
    }
}
...
infinicore::Tensor Glm4MoeLiteMoE::forward(infinicore::Tensor hidden_states) const {
    ...
    auto routed = legacy_experts_->forward(flat_hidden_states, selected_experts, routing_weights);
