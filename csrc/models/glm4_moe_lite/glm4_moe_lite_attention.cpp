...
infinicore::Tensor Glm4MoeLiteAttention::forward_mha_v2(
    const infinicore::Tensor &q,
    const infinicore::Tensor &kv_c,
    const infinicore::Tensor &k_pe,
    size_t tokens) const {
...
infinicore::Tensor Glm4MoeLiteAttention::forward(const infinicore::Tensor &positions,
    ...
    infinicore::Tensor q_lora, kv_c, k_pe;
    {
        ...
        kv_c = kv_latent->narrow({{1, 0, kv_lora_rank_}});
        kv_c = kv_a_layernorm_->forward(kv_c);

        k_pe = kv_latent->narrow({{1, kv_lora_rank_, qk_rope_head_dim_}})->contiguous();
    }
    ...
    // 优化点：同时支持q_pe和k_pe的算子，支持非连续的k_pe
    apply_rope_(positions->view({positions->numel()}),
                q_pe,
                k_pe->unsqueeze(1));
    ...
    if (is_prefill) {
        auto output = forward_mha_v2(q, kv_c, k_pe, tokens);
    ...
    auto [attn_latent_4d, _] = mla_attn_->forward(q_flash, sched_meta);
