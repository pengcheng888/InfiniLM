...
infinicore::Tensor DeepseekV4DecoderLayer::forward(
    ...
    auto attn_normed = attn_norm_->forward(attn_in);
    auto attn_out = attn_->forward(positions, attn_normed);
    ...
    auto ffn_normed = ffn_norm_->forward(ffn_in);
    auto ffn_out = ffn_->forward(ffn_normed, input_ids);
