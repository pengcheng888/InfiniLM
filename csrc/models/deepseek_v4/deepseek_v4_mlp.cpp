...
infinicore::Tensor DeepseekV4MLP::forward(infinicore::Tensor hidden_states) const {
    return forward_linear(hidden_states);
}

infinicore::Tensor DeepseekV4MLP::forward_linear(infinicore::Tensor hidden_states) const {
...
infinicore::Tensor DeepseekV4PackedMLP::forward_packed(infinicore::Tensor hidden_states) const {
    ...
    if (!metadata_ready_ || metadata_tokens_ != tokens) {
        infinicore::op::deepseek_v4_shared_experts_impl_int8_marlin_prepare_metadata_(
            sorted_token_ids,
            expert_ids,
            num_tokens_post_pad,
            topk_weights,
            tokens);
        metadata_tokens_ = tokens;
    ...
    infinicore::op::deepseek_v4_shared_experts_impl_int8_marlin_workspace_(
        output,
        hidden_states,
        w13_weight_marlin_,
        w2_weight_marlin_,
