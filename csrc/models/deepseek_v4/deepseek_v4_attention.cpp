...
infinicore::Tensor DeepseekV4Attention::compute_sparse_attention(const infinicore::Tensor &q,
    ...
    auto *cache = dynamic_cast<infinilm::layers::mla_attention::SparseFlashMLACache *>(
        forward_context.flashmla_cache_vec[layer_idx_].get());
    if (cache == nullptr) {
        throw std::runtime_error("DeepseekV4Attention requires a SparseFlashMLACache");
    }
    ...
    cache->set_key_buffer(kv, forward_context.swa_attn_metadata.raw_out_loc);
