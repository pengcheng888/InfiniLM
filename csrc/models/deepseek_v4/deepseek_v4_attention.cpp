...
infinicore::Tensor DeepseekV4Attention::forward(const infinicore::Tensor &positions,
    ...
    infinicore::Tensor q_for_flash = q;
    infinicore::Tensor attn_out_for_flash;
    size_t local_head_start = 0;
    if (tp_size_ > 1) {
        local_head_start = tp_rank_ * num_local_attention_heads_;
        q_for_flash = infinicore::Tensor::zeros({seq_len, num_attention_heads_, head_dim_}, hidden_states->dtype(), device);
        q_for_flash->narrow({{1, local_head_start, num_local_attention_heads_}})->copy_from(q);
        attn_out_for_flash = infinicore::Tensor::zeros({seq_len, num_attention_heads_, head_dim_}, hidden_states->dtype(), device);
    } else {
        attn_out_for_flash = infinicore::Tensor::zeros({seq_len, num_local_attention_heads_, head_dim_}, hidden_states->dtype(), device);
    }

    infinicore::op::deepseek_v4_flashmla_sparse_attention_(q_for_flash,
                                                           layer_cache.swa_cache_raw,
                                                           swa_indices,
                                                           swa_topk_lengths,
                                                           attn_sink_,
    ...
    auto attn_out = attn_out_for_flash;
    if (tp_size_ > 1) {
        attn_out = attn_out_for_flash->narrow({{1, local_head_start, num_local_attention_heads_}})->contiguous();
    }
