...
| `INFINILM_FUSED_REPLICATED_LINEAR_W8A8_BACKEND` | `native` | `native`, `linear_w8a8i8`, `lmslim`, `lmslim_rocblas`, `rocblas`, `lmslim_hipblaslt`, `hipblaslt`, `hipblaslt_channelwise`, `lightop_smooth`, `lmslim_lightop`, `off`, `none`, `0`, `false` | `FusedReplicatedLinear::forward_split` 静态初始化 | 控制 DeepSeek V4 形状 `M>16,N=1536,K=4096` 的 fused replicated linear 是否走外部 workspace 版 W8A8 linear backend。`native` 调用 InfiniCore `linear_w8a8i8_out_workspace_`；`lmslim` 调用 InfiniCore `deepseek_v4_lmslim_linear_w8a8_`，用 lightop 的 per-token dynamic quant 再接 `scaled_mm_i8_`；`lmslim_rocblas` 调用 InfiniCore `deepseek_v4_lmslim_rocblas_linear_w8a8_`，用 lightop 的 per-token dynamic quant、lmslimquant 导出的 rocBLAS INT8 GEMM 和外部 I32 accum workspace 完成计算；`lmslim_hipblaslt` 调用 InfiniCore `deepseek_v4_lmslim_hipblaslt_channelwise_linear_w8a8_`，用 lightop per-token dynamic quant 后接 lmslimquant 的 `hipblaslt_gemm_channelwise`，当前 SO 接口返回新 Tensor 后再 `copy_` 到外部 output；`lightop_smooth` 调用 InfiniCore `deepseek_v4_lightop_linear_w8a8_smooth_`，即 per-token dynamic quant 后接 lightop `gemm_w8a8_smooth_asm`，并保留外部 output/q_input/input_scale workspace。当前 lightop smooth config 未覆盖 `N=1536,K=4096`，且端到端 TP8 命中 SO GEMM 后仍会崩溃，因此该后端需要额外设置 `INFINILM_FUSED_REPLICATED_LINEAR_ALLOW_EXPERIMENTAL_LIGHTOP_SMOOTH=1`，仅用于隔离调试。`lightop_asm` blockwise 路径端到端会 segfault，因此模型侧仍显式禁用。 |
| `INFINILM_FUSED_REPLICATED_LINEAR_ALLOW_EXPERIMENTAL_LIGHTOP_SMOOTH` | 关闭 | `1` 开启 | `FusedReplicatedLinear::forward_split` 静态初始化 | 显式允许实验性的完整 lightop smooth linear 后端。单算子 eager/graph 已通过，但 InfiniLM 端到端仍不稳定，普通推理应继续使用 `native` 或 `lmslim`。 |
| `INFINILM_FUSED_REPLICATED_LINEAR_LMSLIM` | 开启 | 首字符为 `0`, `f`, `F`, `n`, `N` 时关闭 | `FusedReplicatedLinear::forward_split` 静态初始化 | 兼容旧开关。仅当 `INFINILM_FUSED_REPLICATED_LINEAR_W8A8_BACKEND` 未设置时生效；开启等价于 `native`，关闭等价于 `off`。 |
| `INFINILM_FUSED_REPLICATED_LINEAR_PROFILE` | 关闭 | 任意非空且首字符不是 `0` 的值 | `FusedReplicatedLinear::forward_split` 静态初始化 | 统计 W8A8 workspace linear 路径的 hits/misses 和命中 token 数，不做 GPU 同步。 |
| `INFINILM_FUSED_REPLICATED_LINEAR_TIMING` | 关闭 | 任意非空且首字符不是 `0` 的值 | `FusedReplicatedLinear::forward_split` 静态初始化 | 对 W8A8 workspace linear 路径做 GPU-synced timing；会显著扰动 graph/端到端性能，仅用于局部定位。 |
