...
from .configuration_fm9g import FM9GConfig
...
FM9GRMSNorm = infinicore.nn.RMSNorm


class FM9GMLP(infinicore.nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.gate_proj = infinicore.nn.Linear(
            hidden_size, intermediate_size, bias=False, **kwargs
        )
        self.up_proj = infinicore.nn.Linear(
            hidden_size, intermediate_size, bias=False, **kwargs
        )
        self.down_proj = infinicore.nn.Linear(
            intermediate_size, hidden_size, bias=False, **kwargs
        )
        self.act_fn = infinicore.nn.functional.silu

    def forward(self, hidden_state: infinicore.Tensor) -> infinicore.Tensor:
        return self.down_proj(
            self.act_fn(self.gate_proj(hidden_state)) * self.up_proj(hidden_state)
        )


class FM9GAttention(infinicore.nn.Module):
    def __init__(self, config: FM9GConfig, layer_idx: int, **kwargs):
        super().__init__()
        self.config = config
...
class FM9GDecoderLayer(infinicore.nn.Module):
    def __init__(self, config: FM9GConfig, layer_idx: int, **kwargs):
        super().__init__()
        hidden_size = config.hidden_size
        rms_norm_eps = config.rms_norm_eps

        self.self_attn = FM9GAttention(config=config, layer_idx=layer_idx, **kwargs)
        self.mlp = FM9GMLP(config=config, **kwargs)

        self.input_layernorm = FM9GRMSNorm(hidden_size, eps=rms_norm_eps, **kwargs)
        self.post_attention_layernorm = FM9GRMSNorm(
...
class FM9GModel(infinicore.nn.Module):
    def __init__(self, config: FM9GConfig, **kwargs):
        super().__init__()
        self.config = config
        ...
        self.layers = infinicore.nn.ModuleList(
            [
                FM9GDecoderLayer(config, layer_idx, **kwargs)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = FM9GRMSNorm(config.hidden_size, eps=config.rms_norm_eps, **kwargs)
...
class FM9GForCausalLM(infinicore.nn.Module, GenerationMixin):
    config: FM9GConfig
    ...
    def __init__(self, config, **kwargs):
        ...
        self.model = FM9GModel(config, **kwargs)
    ...
    def from_pretrained(
        ...
        return FM9GForCausalLM(config, device=device, dtype=dtype)
