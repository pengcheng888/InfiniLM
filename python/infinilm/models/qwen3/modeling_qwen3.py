...
Qwen3RMSNorm = infinicore.nn.RMSNorm


class Qwen3MLP(infinicore.nn.Module):
...
class Qwen3Attention(infinicore.nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int, **kwargs):
        super().__init__()
        self.config = config
        ...
        self.q_norm = Qwen3RMSNorm(self.head_dim, eps=config.rms_norm_eps, **kwargs)
        self.k_norm = Qwen3RMSNorm(self.head_dim, eps=config.rms_norm_eps, **kwargs)
        ...
        **kwargs,
        ...
        query_states = self.q_norm(
            self.q_proj(hidden_states).view(
                (querys_shape[0] * querys_shape[1] * querys_shape[2], querys_shape[3])
            )
        ).view(querys_shape)
        ...
        key_states = self.k_norm(
            self.k_proj(hidden_states).view(
                (keys_shape[0] * keys_shape[1] * keys_shape[2], keys_shape[3])
            )
        ).view(keys_shape)
...
class Qwen3DecoderLayer(infinicore.nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int, **kwargs):
        super().__init__()
        hidden_size = config.hidden_size
        ...
        self.mlp = Qwen3MLP(config=config, **kwargs)

        self.input_layernorm = Qwen3RMSNorm(hidden_size, eps=rms_norm_eps, **kwargs)
...
class Qwen3Model(infinicore.nn.Module):
    def __init__(self, config: Qwen3Config, **kwargs):
        ...
        self.layers = infinicore.nn.ModuleList(
            [
                Qwen3DecoderLayer(config, layer_idx, **kwargs)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps, **kwargs)
...
class Qwen3ForCausalLM(infinicore.nn.Module, GenerationMixin):
    config: Qwen3Config
    ...
    def from_pretrained(
        ...
        return Qwen3ForCausalLM(config, device=device, dtype=dtype)
